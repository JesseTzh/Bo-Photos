    ...
    <div className="columns-[260px] gap-3">
      {assets.map((item) => (
        ...
        >
          ...
          />
          <div className="absolute inset-x-0 bottom-0 translate-y-2 bg-gradient-to-t from-media-scrim/80 to-transparent px-4 pb-4 pt-12 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
            <strong className="block truncate text-sm font-medium text-on-media">{item.title || item.original_name}</strong>
            <span className="mt-1 block truncate text-xs text-on-media/70">{[item.camera, item.lens].filter(Boolean).join(" · ")}</span>
          </div>
        </AppLink>
      ))}
    </div>
